...
void UIManager::initStartMenu()
{
	...
	sf::Text text(L"ПРОДОЛЖИТЬ ИГРУ", Resources::UIFont, 50.0f);
...
void UIManager::initGameMenu()
{
	...
	sf::Text text(L"ПРОДОЛЖИТЬ", Resources::UIFont, 50.0f);
	sf::RectangleShape shape({ text.getLocalBounds().width + 20.0f, 60.0f });
	shape.setFillColor(sf::Color(100, 100, 100));
	Button button(Group(shape, text));
	button.setPosition({ SCREEN_W / 2.0f, 2.0f * SCREEN_H / 3.0f - 35.0f });
...
void UIManager::initSetting()
{
	...
	sf::Text text(L"ВЫХОД", Resources::UIFont, 50.0f);
	sf::RectangleShape shape({ text.getLocalBounds().width + 20.0f, 60.0f });
	shape.setFillColor(sf::Color(100, 100, 100));
	Button button(Group(shape, text));
...
{
	sf::Text text(L"Баланс: " + std::to_wstring(player->money) + L" | Запчасти: " + std::to_wstring(player->details) + L" | Одно улучшение стоит 50 руб и 15 деталей", Resources::UIFont, 30.0f);
