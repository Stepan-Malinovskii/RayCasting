...
const int WindowWight = 320, WindowHight = 200;
...
private:
    Player* player;
    Map* nowPlace;
    std::vector<std::pair<sf::Vector2f, sf::Vector2f>>* masRays;
    int RayCount = WindowWight;

    float rayX, rayY, dist;
    bool hitWall;
    sf::Vector2f test;

    sf::Vector2f Ray(float rayAngle)
    {
...
public:
    RayCasting(Player* pl, Map* nmp, std::vector<std::pair<sf::Vector2f, sf::Vector2f>>* rays) : player{ pl }, nowPlace{ nmp }, masRays{ rays } {}
    ...
    void CastRay()
    {
        ...
        {
            angle = player->Angle - player->Fov / 2 + player->Fov * cnt / RayCount;
            masRays->push_back({ player->Position, Ray(angle)});
        }
    }
...
private:
    ...
    sf::Vertex line[2];
    std::vector<std::pair<sf::Vector2f, sf::Vector2f>>* distsToWall;
...
public:
    GameManager(sf::RenderWindow* _window) : window{ _window }
    {
        ...
        distsToWall = new std::vector<std::pair<sf::Vector2f, sf::Vector2f>>();
        rayCasting = new RayCasting(player, ((*maps)[nowLocation]), distsToWall);
    ...
    void Draw()
    {
        ...
        for (auto fg : (*maps)[nowLocation]->Figure)
        {
            window->draw(*fg);
        }

        for (auto i : *distsToWall)
        {
            line[0] = i.first;
            line[1] = i.second;
...
int main1()
{
    ...
    sf::RenderWindow window(sf::VideoMode(WindowWight, WindowHight), "My window", sf::Style::Fullscreen, setings);
